...
const CONFIG = {
  ...
  POLAROIDS: [
    { caption: 'A walk to remember, just you and me', rotate: -5, url: 'images/foto1.jpg' },
    { caption: 'Finding our inner child and endless joy together', rotate: 4, url: 'images/foto2.jpg' },
    { caption: 'Cute moments captured with my favorite person', rotate: -3, url: 'images/foto3.jpg' }
  ],
...
function pinKeyHandler(e) {
  ...
  if (e.key >= '0' && e.key <= '9') {
    markUserInteraction();
    const btn = qs(`.pin-btn[data-digit="${e.key}"]`);
    if (btn) {
      btn.classList.add('pressed');
      setTimeout(() => btn.classList.remove('pressed'), 150);
    }
...
function clearPinErrorState() {
  if (pinErrorTimeout) {
    clearTimeout(pinErrorTimeout);
    pinErrorTimeout = null;
  }
...
function pinSuccess() {
  ...
  setTimeout(() => {
    const pinScreen = $('pin-screen');
    if (pinScreen) {
      pinScreen.style.transition = 'opacity 0.7s ease, transform 0.7s ease';
      pinScreen.style.opacity = '0';
      pinScreen.style.transform = 'scale(1.04)';
      setTimeout(() => {
...
/* ─── DEV SHORTCUT — REMOVE BEFORE SENDING FINAL LINK ───────── */
...
function playGiftVideoModal() {
  ...
  const videoPetalsContainer = $('video-modal-petals');

  function spawnVideoPetal() {
    if (!videoPetalsContainer) return;
    if (videoPetalsContainer.childElementCount >= 14) return;

    const petalEmojis = ['🌸', '🌺', '🌷', '🌼', '✿', '❀'];
    const p = document.createElement('div');
    p.className = 'video-modal-petal';
    p.textContent = petalEmojis[randInt(0, petalEmojis.length - 1)];

    const size = rand(16, 32);
    const leftPercent = rand(2, 96);
    ...
    videoPetalsContainer.appendChild(p);

    setTimeout(() => {
      if (p.parentNode === videoPetalsContainer) {
        p.remove();
      }
    }, (duration + 0.5) * 1000);
  }
  ...
  if (videoPetalsContainer) {
    ...
    for (let i = 0; i < 5; i++) {
      setTimeout(spawnVideoPetal, i * 180);
    }
    videoPetalsInterval = setInterval(spawnVideoPetal, 450);
  }
...
function initPolaroids() {
  ...
  CONFIG.POLAROIDS.forEach((photo, i) => {
    ...
    el.innerHTML = `
      <div class="polaroid-tape" style="position:absolute;top:-8px;left:20%;width:60px;height:24px;background:rgba(255,150,180,0.25);border:1px solid rgba(255,180,200,0.3);border-radius:2px;transform:rotate(-5deg);"></div>
...
function initMusicFloatBtn() {
  const btn = $('music-float-btn');
  if (!btn) return;
